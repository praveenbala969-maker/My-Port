  ...
  return (
    <div className="bg-white border border-blue-100 rounded-3xl p-4 sm:p-8 shadow-xs space-y-7 scroll-mt-20">
          ...
          <div className="flex bg-white p-1 rounded-xl border border-neutral-200 text-xs shadow-xs">
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg font-bold transition-all ${
                viewMode === 'slider' ? 'bg-[#0694D1] text-white shadow-xs' : 'text-neutral-600 hover:text-neutral-900'
              }`}
              title="Interactive drag slider"
            >
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg font-bold transition-all ${
                viewMode === 'side-by-side' ? 'bg-[#0694D1] text-white shadow-xs' : 'text-neutral-600 hover:text-neutral-900'
              }`}
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg font-bold transition-all ${
                viewMode === 'toggle' ? 'bg-[#0694D1] text-white shadow-xs' : 'text-neutral-600 hover:text-neutral-900'
              }`}
      ...
      <div className="space-y-3">
          ...
          <div className="flex items-center gap-3 text-[11px]">
            {viewMode === 'slider' && (
              <span className="text-slate-400 font-mono hidden sm:inline">
                Drag divider: <strong className="text-white">{Math.round(sliderPosition)}% Redesign</strong>
              </span>
            )}
            {viewMode === 'toggle' && (
              <div className="flex items-center gap-1 bg-slate-800 p-0.5 rounded-lg">
                <button
                  ...
                  className={`px-2.5 py-0.5 rounded text-[10px] font-bold ${
                    toggleState === 'before' ? 'bg-rose-900/80 text-rose-200' : 'text-slate-400'
                  }`}
                >
                  Legacy (Before)
                </button>
                <button
                  ...
                  className={`px-2.5 py-0.5 rounded text-[10px] font-bold ${
                    toggleState === 'after' ? 'bg-[#0694D1] text-white' : 'text-slate-400'
                  }`}
                >
                  Redesigned (After)
                </button>
        ...
        {/* The Comparison Stage Container */}
        <div
          ref={containerRef}
          onClick={(e) => {
            if (viewMode === 'slider') {
          ...
          className="relative w-full rounded-b-2xl border-x border-b border-neutral-200 bg-neutral-900 shadow-xl overflow-hidden min-h-[580px] cursor-pointer"
        >
          {/* 1. SLIDER MODE */}
          {viewMode === 'slider' && (
            <div className="relative w-full h-full min-h-[580px] select-none">
              {/* Layer 1: Underneath (BEFORE - Legacy) */}
              <div className="absolute inset-0 w-full h-full overflow-hidden bg-neutral-950 flex items-center justify-center">
                {activeBeforeImage ? (
                  <img
                    src={activeBeforeImage}
                    alt={`${currentScreenConfig.label} Legacy Screenshot`}
                    referrerPolicy="no-referrer"
                    className={`w-full h-full ${
                      zoomFit === 'contain' ? 'object-contain' : 'object-cover'
                    } object-top`}
                  />
                ) : (
                ...
                <div className="absolute top-4 left-4 z-10 px-3 py-1 bg-rose-600/90 text-white text-[11px] font-black uppercase tracking-wider rounded-full shadow-md backdrop-blur-xs flex items-center gap-1.5 pointer-events-none">
                  <span className="w-2 h-2 rounded-full bg-white animate-pulse" />
                  <span>Legacy (Before)</span>
                </div>
              </div>

              {/* Layer 2: On Top with Clip Path (AFTER - Redesigned Screenshot) */}
              <div
                className="absolute inset-0 w-full h-full overflow-hidden pointer-events-none bg-neutral-950"
                style={{
              ...
              >
                <div className="w-full h-full pointer-events-auto flex items-center justify-center">
                  {activeAfterImage ? (
                    <img
                      ...
                      alt={`${currentScreenConfig.label} Redesign Screenshot`}
                      referrerPolicy="no-referrer"
                      className={`w-full h-full ${
                        zoomFit === 'contain' ? 'object-contain' : 'object-cover'
                      } object-top`}
                ...
                <div className="absolute top-4 left-4 z-10 px-3 py-1 bg-[#0694D1]/90 text-white text-[11px] font-black uppercase tracking-wider rounded-full shadow-md backdrop-blur-xs flex items-center gap-1.5 pointer-events-none">
                  <span className="w-2 h-2 rounded-full bg-white animate-pulse" />
                  <span>Redesigned (After: {currentScreenConfig.fileName})</span>
                </div>
              </div>
              ...
              >
                ...
                <div className="w-0.5 h-full bg-white shadow-[0_0_12px_rgba(0,0,0,0.8)] group-hover:bg-[#0694D1] transition-colors" />

                {/* Center Handle Button */}
                <div className="absolute w-10 h-10 rounded-full bg-slate-900 border-2 border-white shadow-2xl text-white flex items-center justify-center group-hover:scale-110 group-active:scale-95 transition-transform">
                  <MoveHorizontal className="w-4 h-4 text-[#0694D1]" />
                </div>
              </div>

              {/* Callout Pins Placed Over Redesigned Layer */}
              {showHotspots && (
                  ...
                  {currentScreenConfig.hotspots.map((hotspot, idx) => (
                    ...
                    >
                      <div className="relative flex items-center justify-center">
                        <span className="absolute w-6 h-6 rounded-full bg-[#0694D1] opacity-75 animate-ping" />
                        <div
          ...
          {viewMode === 'side-by-side' && (
            <div className="grid grid-cols-1 lg:grid-cols-2 divide-y lg:divide-y-0 lg:divide-x divide-neutral-800 min-h-[580px]">
              {/* Left Column: BEFORE (Legacy) */}
              <div className="relative bg-neutral-950 p-2 flex flex-col justify-between">
                <div className="flex items-center justify-between pb-2 px-2 text-xs border-b border-neutral-800 text-neutral-400">
                  <span className="font-bold text-rose-400 flex items-center gap-1.5">
                    <span className="w-2 h-2 rounded-full bg-rose-500" /> Legacy Experience (Before)
                  </span>
                  {activeBeforeImage && (
                    <button
                ...
                <div className="relative flex-1 flex items-center justify-center my-2 min-h-[500px]">
                  {activeBeforeImage ? (
                    <img
                      src={activeBeforeImage}
              ...
              {/* Right Column: AFTER (Redesigned Screenshot) */}
              <div className="relative bg-neutral-950 p-2 flex flex-col justify-between">
                <div className="flex items-center justify-between pb-2 px-2 text-xs border-b border-neutral-800 text-neutral-400">
                  <span className="font-bold text-sky-400 flex items-center gap-1.5">
                    <span className="w-2 h-2 rounded-full bg-[#0694D1]" /> Redesigned Experience ({currentScreenConfig.fileName})
                  </span>
                  {activeAfterImage && (
                    <button
          ...
          {/* 3. TOGGLE VIEW MODE */}
          {viewMode === 'toggle' && (
            <div className="relative w-full h-full min-h-[580px] bg-neutral-950 flex items-center justify-center">
              {toggleState === 'after' ? (
                <div className="w-full h-full flex items-center justify-center">
                  ...
                  <div className="absolute top-4 left-4 z-10 px-3 py-1 bg-[#0694D1] text-white text-[11px] font-black uppercase rounded-full shadow-md">
                    Redesigned (After: {currentScreenConfig.fileName})
                  </div>
                </div>
              ) : (
                <div className="w-full h-full flex items-center justify-center">
                  ...
                  <div className="absolute top-4 left-4 z-10 px-3 py-1 bg-rose-600 text-white text-[11px] font-black uppercase rounded-full shadow-md">
                    Legacy (Before)
                  </div>
                </div>
              )}
